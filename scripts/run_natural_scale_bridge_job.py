...
def main() -> None:
    ...
    ap.add_argument("--matrix", default=os.environ.get("AFM_NATURAL_BRIDGE_MATRIX", "runs_compatibility_natural_scale_bridge_v1/job_matrix.json"))
    ...
    if not torch.cuda.is_available():
        raise SystemExit("natural-scale causal bridge requires CUDA")

    runner = NaturalScaleBridgeRunner(
        source_run_dir=row["source_run_dir"],
        run_dir=row["run_dir"],
        system=row["system"],
        device=torch.device("cuda"),
        natural_median_update_norm=float(row["natural_median_update_norm"]),
        natural_norm_fractions=[float(x) for x in row["natural_norm_fractions"]],
